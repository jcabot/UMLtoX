...
/**
 *  Export for Poseidon v 5 files
 * 
 * @version 0.1 April 2009
 * @author jcabot
 *
 */
public class ExportXMIPoseidon extends ExportXMI{

	public ExportXMIPoseidon() {super();}
	
	protected void exportProjectInfo()
	{
		out.println("<XMI xmi.version = '1.2' xmlns:UML = 'org.omg.xmi.namespace.UML' xmlns:UML2 = 'org.omg.xmi.namespace.UML2'>");
		out.println("<XMI.header>");
		out.println("<XMI.documentation>");
		out.println("<XMI.exporter> Netbeans XMI Writer </XMI.exporter>");
		out.println("<XMI.exporterVersion>1.0 </XMI.exporterVersion>");
		out.println("<XMI.metaModelVersion>1.4.5</XMI.metaModelVersion>");
		out.println("</XMI.documentation>");
		out.println("</XMI.header>");
	};
	...
	protected void exportStaticModel(StaticModel s)
	{
	  ...
	  out.println("<UML:Model xmi.id =" + "'" +newID() + "'" + " name = '"+ s.getName() + "'" + " isSpecification = 'false' isRoot = 'false' isLeaf = 'false' isAbstract = 'false'>");
	  incNested();
	...
	{
		Iterator<Association> it=p.getAllAssociationsNoAssCl().iterator();
		while (it.hasNext())
		{
	...
	//    if (at.getStereotypes().size()>0) exportRefStereotype(at);
	    if (at.getType()!=null)
	    {
	    	...
	    	out.println(nestedString()+ "<UML2:TypedElement.type>");
	    	incNested();
...
//		   if (ascl.getStereotypes().size()>0) exportRefStereotype(ascl);
	...
	protected void exportDataTypes(Vector<DataType> dts)
	{
		...
		while (it.hasNext())
		{
		  ...
		  {
			  if (enumStereotypeId==null)
			  {
				enumStereotypeId=(new Integer(newID())).toString();
			  }
			  Enumeration e= (Enumeration) dt;
			  out.println(nestedString()+"<UML:Class xmi.id = '" + id +"' name = '" + e.getName() + "' visibility = 'public' isSpecification = 'false' isRoot = 'false' "
					  + " isLeaf = 'false' isAbstract = 'false'" + " isActive = 'false'>");
			  incNested();
			  out.println(nestedString() + "<UML:ModelElement.stereotype>");
			  incNested();
			  out.println(nestedString() + "<UML:Stereotype xmi.idref = '" + enumStereotypeId +"'/>");
			  decNested();
			  out.println(nestedString() + "</UML:ModelElement.stereotype>");
	          out.println(nestedString()+"<UML:Classifier.feature>");
			  incNested();
			  Iterator<String> itLit=e.getValues().iterator();
			  while (itLit.hasNext())
			  {
				String literal=itLit.next();
				//int idDType=Integer.parseInt(hash.get(at.getType()));
				out.println(nestedString() + "<UML:Attribute xmi.id = '" + newID()+"' name = '" + literal + "' visibility = 'public' isSpecification = 'false' ownerScope = 'instance' " +
	...
	//		    out.println(nestedString()+ "<UML2:TypedElement.type>");
		//	    incNested();
		  ...
		  else out.println(nestedString() + "<UML:DataType xmi.id = '" + id + "' name = '" + dt.getName()+ "' isSpecification = 'false' isRoot = 'false' isLeaf = 'false' isAbstract = 'false'/> ");
		}
		//If we have any enumeration in the model, we must create the corresponding stereotype element
		if  (enumStereotypeId!=null) 
		{
			out.println(nestedString() +  "<UML:Stereotype xmi.id = '" + enumStereotypeId + "' name = 'enumeration' visibility = 'public' isSpecification = 'false' isRoot = 'false'" +
					" isLeaf = 'false' isAbstract = 'false'>");
			incNested();
			out.println(nestedString() + "<UML:Stereotype.baseClass>Class</UML:Stereotype.baseClass>");
	        decNested();
